...
- [x] **任务组 5：后端任务编排与接口**
  - [x] 设计并迁移数据库表结构（templates、tasks、task_results 等）。
  - [x] 实现任务创建、进度更新、状态查询 API，支持 BackgroundTasks 抽象。
  - [x] 完成结果文件归档、过期清理、下载 token 管理。
  - [x] 提供格式与高级选项查询、模板详情等接口。
  - [x] 写作《后端 API 使用说明.md》，涵盖接口约定、错误码与示例。
- [x] **任务组 6：前端实现与交互**
  - [x] 封装 Axios API 客户端，处理错误拦截、文件下载与通用请求配置。
  - [x] 构建模板列表、预览与说明界面，串联后端模板接口。
  - [x] 实现元数据驱动的动态表单，支持多字段类型与校验提示。
  - [x] 实现输出格式与高级选项配置组件，联动任务提交。
  - [x] 构建任务状态、下载管理与错误重试体验。
  - [x] 整理《前端组件与状态管理指南.md》，沉淀复用规范。
- [ ] **任务组 7：综合验证与交付**
